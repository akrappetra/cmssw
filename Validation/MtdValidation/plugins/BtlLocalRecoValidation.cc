...
private:
  ...
  //edm::EDGetTokenT<FTLSuperClusterCollection> btlSuperCluToken_;
  ...
  // superclusters
  /*MonitorElement* meNsuperclusters_;
  MonitorElement* meSuperclusterEnergy_;
  MonitorElement* meSuperclusterTime_;
  MonitorElement* meSuperclusterTimeError_;
  MonitorElement* meSuperclusterX_;
  MonitorElement* meSuperclusterY_;
  MonitorElement* meSuperclusterNClusters_;
  MonitorElement* meSuperclusterEta_;
  MonitorElement* meSuperclusterPhi_;
  MonitorElement* meSuperclusterEnergyVsEta_;
  MonitorElement* meSuperclusterNClustersVsEta_;*/
...
BtlLocalRecoValidation::BtlLocalRecoValidation(const edm::ParameterSet& iConfig)
  ...
  //btlSuperCluToken_ = consumes<FTLSuperClusterCollection>(iConfig.getParameter<edm::InputTag>("recSuperCluTag"));
...
void BtlLocalRecoValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // --- Loop over the BTL superclusters ---
  /*auto btlSuperCluHandle = makeValid(iEvent.getHandle(btlSuperCluToken_));
  unsigned int n_superclus_btl(0);

  for (const auto& superCluster : *btlSuperCluHandle) {
    if (superCluster.energy() < hitMinEnergy_)
      continue; 
    
    n_superclus_btl++;

    BTLDetId superCluId = BTLDetId(superCluster.id().rawId());
    DetId detIdObject(superCluId);
    const auto& genericDet = geom->idToDetUnit(detIdObject);

    if (genericDet == nullptr) {
      throw cms::Exception("BtlLocalRecoValidation")
          << "GeographicalID: " << std::hex << superCluId << " is invalid!" << std::dec << std::endl;
    }

    Local3DPoint local_point(superCluster.x(), superCluster.y(), 0.0); // needs to be checked?? taking the first cluster position?
    const auto& global_point = genericDet->toGlobal(local_point);
  
    meSuperclusterEnergy_->Fill(superCluster.energy());
    meSuperclusterTime_->Fill(superCluster.time());
    meSuperclusterPhi_->Fill(global_point.phi());
    meSuperclusterEta_->Fill(global_point.eta());
    meSuperclusterX_->Fill(global_point.x());
    meSuperclusterY_->Fill(global_point.y());
    meSuperclusterNClusters_->Fill(superCluster.nClusters());

    meSuperclusterEnergyVsEta_->Fill(std::abs(global_point.eta()), superCluster.energy());
    meSuperclusterNClustersVsEta_->Fill(std::abs(global_point.eta()), superCluster.nClusters());

  }

  if (n_superclus_btl > 0) {
    meNsuperclusters_->Fill(n_superclus_btl);
  }*/
...
void BtlLocalRecoValidation::bookHistograms(DQMStore::IBooker& ibook,
  ...
  // superclusters
  /*meNsuperclusters_ = ibook.book1D("BtlNsuperclusters", "Number of BTL RECO superclusters;N_{RECO}", 10, 0., 10.);
  meSuperclusterEnergy_ = ibook.book1D("BtlSuperclusterEnergy", "BTL RECO superclusters energy;E_{RECO} [MeV]", 20, 0., 20.);
  meSuperclusterTime_ = ibook.book1D("BtlSuperclusterTime", "BTL RECO superclusters ToA;ToA_{RECO} [ns]", 25, 0., 25.);
  meSuperclusterTimeError_ = ibook.book1D("BtlSuperclusterTimeError", "BTL RECO superclusters T error;#sigma^{T}_{RECO} [ns]", 50, 0., 0.1);
  meSuperclusterX_ = ibook.book1D("BtlSuperclusterX", "BTL RECO superclusters X;X_{RECO} [cm]", 100, -10., 10.);
  meSuperclusterY_ = ibook.book1D("BtlSuperclusterY", "BTL RECO superclusters Y;Y_{RECO} [cm]", 60, -3., 3.);
  meSuperclusterEta_ = ibook.book1D("BtlSuperclusterEta", "BTL RECO superclusters #eta;#eta_{RECO}", 100, -1.55, 1.55);
  meSuperclusterPhi_ = ibook.book1D("BtlSuperclusterPhi", "BTL RECO superclusters #phi;#phi_{RECO} [rad]", 126, -3.2, 3.2);
  meSuperclusterEnergyVsEta_ = ibook.bookProfile("BtlSuperclusterEnergyVsEta", "BTL RECO superclusters energy vs #eta;#eta_{RECO};E_{RECO} [MeV]", 50, -1.55, 1.55, 0., 100.);
  meSuperclusterNClustersVsEta_ = ibook.bookProfile("BtlSuperclusterNClustersVsEta", "BTL RECO superclusters n. clusters vs #eta;#eta_{RECO};N_{clusters}", 50, -1.55, 1.55, 0., 100.);
*/
...
void BtlLocalRecoValidation::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  //desc.add<edm::InputTag>("recSuperCluTag", edm::InputTag("mtdSuperClusters", "FTLBarrel"));
